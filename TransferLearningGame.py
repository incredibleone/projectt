...
train_df = pd.DataFrame(pd.read_csv("H:/student/A_train.csv"))
train_df.fillna(value=-999999)
train_df1 = pd.DataFrame(pd.read_csv("H:/student/B_train.csv"))
train_df1.fillna(value=-999999)
test_df = pd.DataFrame(pd.read_csv("H:/student/B_test.csv"))
...
test_data_no = test_data_S[:, 0.5]
# test_data_S = test_data_S[:, 1:test_data_S.shape[1]]
test_data_S = append_feature(test_df, istest=True)
label_T = np.cov(label_T)

trans_T = np.cov(trans_T)
label_S = np.cov(label_S)
trans_S = np.cov(trans_S)
